        ...
        <div>
            <header className={`${isActive ? 'bg-black py-[2px]' : 'bg-transparent py-[3px]'} ${!isHomeRoute ? "border-b-2" : ""} fixed w-full z-30 left-0 right-0 mx-auto top-0 flex justify-between items-center px-[10px] lg:px-[40px] transition-all duration-300`}>
                <div>
                    ...
                    <div>
                        <Typography variant='h5' className='flex-wrap text-white hover:scale-105 transition-all duration-300 font-nunito font-semibold'>
                            No Gutz No Glory
                        </Typography>
                    </div>
                </div>
                ...
                <div className='hidden lg:flex space-x-2 '>
                    <button className={`btn ${isHomeRoute ? 'btn-sm text-white hover:bg-gray-600' : 'btn-lg hover:text-black'}  rounded-sm  transition-all duration-200 font-nunito`}>Login</button>
                    <button className={`btn btn-sm ${isHomeRoute ? 'btn-primary text-black' : 'btn-secondary bg-black text-white rounded-md '} font-nunito`} onClick={handleOpen}>SignUp</button>
                ...
                <nav className={`${navmobile ? 'h-screen' : 'h-0'} lg:hidden w-full bg-neutral-500 fixed top-0 left-0 right-0 bottom-0 -z-10 overflow-hidden transition-all h-0`} >
                    ...
                    <div className='-mt-44 flex justify-center gap-x-8'>
                        <button className='btn btn-lg  text-white hover:bg-gray-400 transition-all duration-200 font-nunito' onClick={() => setNavMobile(false)}>Log in</button>
